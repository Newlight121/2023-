...
for i = 2:r
    if((((cluster_tmp == 1 && i ~=16) ||cluster_tmp ~= 1)&&(cluster_name=="GMM"))||cluster_name=="Kmeans"&&((cluster_tmp == 5 && i ~=16) ||cluster_tmp ~= 5))
        disp(i)
        a = ED_cluster_time(i,:);
...
% 每个人的残差均值
ED_err_class_tmp_abs_average = [];
for i=2:length(ED_time_class)
    ED_err_class_tmp_abs_average = [ED_err_class_tmp_abs_average; sub_cancha_class_tmp(i,1)/ED_count_class(i)];
end
...
fig = figure(16);hold on;
plot(ED_err_class_tmp_abs_average)
title("聚类"+string(cluster_tmp)+"残差图")
legend('残差值');
ylabel('残差绝对值');
saveas(fig,"聚类"+string(cluster_tmp)+"残差图.png")
...
% 残差求均值
sub_cancha_1_100_average = [];
for i=2:length(ED_time)
    sub_cancha_1_100_average = [sub_cancha_1_100_average; sub_cancha_1_100(i,1)/ED_count(i)];
end
...
ylabel('残差绝对值');
fig = figure(21);
plot(sub_cancha_1_100_average)
disp(sum(sub_cancha_1_100_average)/100)
xlswrite("残差结果.xlsx", sub_cancha_1_100_average);
saveas(fig, "残差图.png")
title("残差图")
legend('残差值');
ylabel('残差绝对值');
