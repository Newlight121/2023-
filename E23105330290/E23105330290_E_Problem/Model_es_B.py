...
for i, model in enumerate(models):
    ...
    pd.DataFrame(np.round(y_ans)).to_csv(r"堆叠模型预测/" + title_name + "模型mRS.csv", mode='w', header=True, encoding='gb18030')
    plt.show()
...
sclf = StackingClassifier(
    estimators=estimatorses[minindex], final_estimator=RandomForestClassifier(n_estimators=1000, random_state=0, n_jobs=-1)
)
